...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def on_process_message(self, message: types.Message, data: dict):
        ...
        try:
            ...
            await self.message_throttled_two(message, t)
            raise CancelHandler()
    ...
    async def message_throttled_two(self, message: types.Message, throttled: Throttled):
        ...
        if throttled.exceeded_count == 2:
            ...
            await message.answer('Всё, чувак, я устал и иду спать! Надеюсь на утро прекратишь писать мне ересь')
        elif throttled.exceeded_count == 4:
            await message.answer('Чел, не пиши мне больше...')
        elif throttled.exceeded_count >= 5:
            await message.answer('... заткнись ...')
        await asyncio.sleep(delta)
